...
<div class="wrapper">
  ...
  <!-- HEADER -->
  <div class="header">
    <div class="pill">⚙ Sistem Order Pedagang</div>
    <h1>Order Form</h1>
    <p>Lengkapi semua field — kirim otomatis ke Discord via Webhook</p>
  </div>


  <!-- CARD 1 — NAMA ANGGOTA -->
  <div class="card">
    <div class="card-title"><span class="icon">👤</span> Nama Anggota</div>
    <div class="field">
  ...
  <!-- CARD 2 — HANDLE -->
  <div class="card">
    <div class="card-title"><span class="icon">🏷️</span> Nama Handle</div>
    <div class="field">
  ...
  <!-- CARD 3 — PAKET -->
  <div class="card">
    <div class="card-title"><span class="icon">📦</span> Pilih Paket</div>
    <div class="pkg-grid" id="pkgGrid"><!-- JS renders --></div>
  </div>

  <!-- CARD 4 — TOTAL -->
  <div class="card">
    <div class="card-title"><span class="icon">💰</span> Total Harga</div>
    <div class="total-row">
  ...
  <!-- CARD 5 — BUKTI FOTO -->
  <div class="card">
    <div class="card-title"><span class="icon">🖼️</span> Bukti Foto Pembayaran</div>
    <div class="upload-zone" id="uploadZone">
      <input type="file" id="fileInput" accept="image/*" multiple />
      <div class="uz-icon">📤</div>
      <div class="uz-title">Drag &amp; Drop atau Klik untuk Upload</div>
      <div class="uz-sub">JPG, PNG, WEBP · Maks 8MB per foto</div>
    </div>
  ...
  <button class="submit-btn" id="submitBtn" disabled>
    ...
    <span class="btn-text">Kirim ke Discord ✈️</span>
  </button>
...
// ─── PACKAGES ───────────────────────────────
const PACKAGES = [
  { id:'small',  name:'Small',  emoji:'🥉', price:50000  },
  { id:'medium', name:'Medium', emoji:'🥈', price:100000 },
  { id:'high',   name:'High',   emoji:'🥇', price:200000 },
  { id:'combo',  name:'Combo',  emoji:'🎯', price:300000 },
  { id:'extra',  name:'Extra',  emoji:'⭐', price:500000 },
];
...
PACKAGES.forEach(pkg => {
  ...
  el.innerHTML = `
    ...
    <div class="pkg-qty">
      <button class="qty-btn" data-action="dec" data-id="${pkg.id}">−</button>
...
function renderPreviews(){
  ...
  state.files.forEach((file,i)=>{
    ...
    reader.onload = ev => {
      wrap.innerHTML = `<img src="${ev.target.result}" alt="bukti" /><div class="thumb-remove" data-idx="${i}">✕</div>`;
    };
...
function showToast(msg, type='success'){
  ...
  icon.innerHTML = type==='loading' ? '<div class="spinner"></div>' : type==='success' ? '✅' : '❌';
...
document.getElementById('submitBtn').addEventListener('click', async ()=>{
  ...
  PACKAGES.forEach(p=>{
    if(state.selected[p.id]){
      ...
      total += sub;
      lines.push(`${p.emoji} **${p.name}** × ${qty}  →  ${formatRp(sub)}`);
    }
  });
  ...
  const embed = {
    ...
    fields:[
      { name:'👤 Nama Anggota', value: nama,   inline:true  },
      { name:'🏷️ Handle',      value: handle, inline:true  },
      { name:'\u200B',          value:'\u200B', inline:true },
      { name:'📦 Paket Dibeli', value: lines.join('\n'),     inline:false },
      { name:'💰 Total Harga',  value: `### ${formatRp(total)}`, inline:false },
    ],
    footer:{
      text:'Pedagang Order System • Auto-sent',
    },
    timestamp: new Date().toISOString()
  };

  showToast('Mengirim data ke Discord…', 'loading');
  ...
  for(let i=0; i<state.files.length; i++){
    ...
    if(i===0){
      formData.append('payload_json', JSON.stringify({
        content: `📸 **Bukti Pembayaran** — ${nama} (${handle})`
      }));
    }
    try {
